...
function EvaluationParameters() {
  ...
  const handleSaveParameter = () => {
    if (!editParameter.name || !editParameter.description) {
    ...
    if (editParameter.id) {
      ...
      setParameters(parameters.map(p => (p.id === editParameter.id ? { ...editParameter } : p)));
      setSuccessMessage(`Parameter "${editParameter.name}" updated successfully!`);
    } else {
      // Add new parameter
      const newParameter = { ...editParameter, id: `param${parameters.length + 1}` };
      setParameters([...parameters, newParameter]);
      setSuccessMessage(`Parameter "${newParameter.name}" added successfully!`);
    }
  ...
  const handleDeleteParameter = () => {
    setParameters(parameters.filter(p => p.id !== selectedParameter.id));
    setShowDeleteModal(false);
    setSuccessMessage(`Parameter "${selectedParameter.name}" deleted successfully!`);
    setTimeout(() => setSuccessMessage(''), 3000);
  };
      ...
      <div className="w-full max-w-7xl mx-auto mt-6 px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {parameters.length > 0 ? (
            parameters.map((param, index) => (
              ...
              >
                <div>
                  ...
                  <p className="text-lg text-white/90">{param.description}</p>
                </div>
            ...
            <p className="text-white/70 text-center col-span-full py-8">No evaluation parameters defined.</p>
          )}
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white/20 backdrop-blur-md p-8 rounded-2xl shadow-glow border border-white/30 w-full max-w-md relative">
            ...
            <h2 className="text-2xl font-bold text-white mb-6 text-center">
              {editParameter.id ? 'Edit Parameter' : 'Add Parameter'}
            </h2>
            ...
            <div className="flex justify-end gap-4 mt-6">
              ...
              <button
                ...
                className="flex items-center bg-gradient-to-r from-accent-teal to-cyan-400 text-white py-2 px-6 sm:px-4 rounded-lg font-semibold hover:bg-opacity-90 hover:scale-105 transition duration-200 shadow-glow border border-white/30 backdrop-blur-md"
              >
                {editParameter.id ? 'Save Changes' : 'Add Parameter'}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white/20 backdrop-blur-md p-8 rounded-2xl shadow-glow border border-white/30 w-full max-w-md relative">
            ...
            <p className="text-white/90 text-center mb-6">
              Are you sure you want to delete the parameter <span className="font-semibold text-accent-teal">"{selectedParameter.name}"</span>? This action cannot be undone.
            </p>
